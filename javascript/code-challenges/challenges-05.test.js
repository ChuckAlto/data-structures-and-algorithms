...
const toLastNames = people => {
  ...
  people.map((arr, idx) =>{
    newList.push(`${arr.firstName} ${arr.lastName}`);
  });
...
const addValues = (arr) => {
  // Solution code here...
  let total = arr.reduce((oldNum, num) =>{
    return oldNum + num;
  }, 0);
  return total;
};
...
const addPurchases = (arr) => {
  ...
  let totalPrice = arr.reduce((total, items) =>{
    return total + items.purchasePrice;
  }, 0);
  return totalPrice;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let totalCount = arr.reduce((total, items, index) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let starNames = arr.reduce((names, list) => {
...
const reversedString = (str) => {
  // Solution code here...
  let reverse = [...str].reduce((revLet, letters) => {
    return letters + revLet;
  }, '');
  return reverse;
};
