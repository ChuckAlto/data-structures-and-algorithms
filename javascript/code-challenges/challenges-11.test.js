...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((firstTotal, arr)=>{
    let newArr = arr.reduce((totalTwo, value)=>{
      if (value === target){
        return totalTwo + 1;
      }
      return totalTwo;
    },0);
    return firstTotal + newArr;
  },0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((total, arr)=> total += arr.reduce((subtotal, value)=> subtotal += value), 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  // let gen = data.find(character => character.gender === 'female' || character.gender === 'male');

  // return data.filter(character => character.gender === 'female' || 'male').map(character => character.name).join(' and ');
};
